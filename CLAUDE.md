...
`<a class="btn google">` and `<a class="btn github">` in `Index.html` point at `/auth/google` and `/auth/github` — a navigation, not a form submit, hence `<a>` and not `<button>`. `script.js` fills `#navCompte` from `/api/moi`; its `/?connexion=…` alerts name no provider, since both flows share those codes. The rest of the signup modal (email/password, Apple, Yahoo) is still decorative.
